...
def main():
    ...
    parser.add_argument('-t', '--use-tts', default=False,
                        help='Use TTS instead of Google Cloud Translate for speech')
...
def cloud_say(msg):
    ...
    play it in mplayer '''
    uri = "http://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&q=%s&tl=en"%msg
    cmd = ['/usr/local/bin/mplayer', '-ao', 'alsa', '-really-quiet',
           '-noconsolecontrols', uri ]
    subprocess.call(cmd)

def do_recognition(args, recorder, recognizer, player):
    """Configure and run the recognizer."""
    if args.use_tts: # this speech audio sucks but works offline
...
class SyncMicRecognizer(object):
    ...
    def _handle_result(self, result):
        if result.transcript and self.actor.handle(result.transcript):
            logger.debug('handled local command: %s', result.transcript)
        elif result.response_audio:
            logger.info('gassist says: %s', '?')
            ...
            #TODO(bryan) flag the saved audio and text, for offline fix
        else:
